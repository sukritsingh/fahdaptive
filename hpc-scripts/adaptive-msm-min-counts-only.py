...
def main():
    ...
    # counts, dtrajs = calc_msm_counts(features)
    ...
    # goals just has to have the right shape so pass dtrajs
    # no goal, mincount only -- set explore_weight=1
    goals = dtrajs
    # with open('current_goal.txt', 'w') as f:
    #     f.write('Current min. dist1: %f' % np.min(dist1))
    #     f.write('\n')
    # no goal, mincount only -- set explore_weight=1
    # If goals matter, then set explore_weight=0.5
    seeds = draw_seeds(goals, counts, dtrajs, n_seeds, explore_weight=1.0, truncate_prob=True)
